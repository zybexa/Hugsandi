...
export default function BlockRenderer({ block, onDataChange, onStyleChange, readOnly, availableImages, defaultShowStyling }: BlockRendererProps) {
  switch (block.data.type) {
    case 'header':
      return <HeaderBlock data={block.data} onChange={onDataChange} blockStyle={block.style} readOnly={readOnly} />;
    case 'content-card':
    ...
    case 'footer':
      return <FooterBlock data={block.data} onChange={onDataChange} blockStyle={block.style} onStyleChange={onStyleChange} readOnly={readOnly} />;
    default:
      return null;
  }
}
